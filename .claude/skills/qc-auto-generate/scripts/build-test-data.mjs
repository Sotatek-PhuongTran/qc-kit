...
if (!varRows) fail('Missing "## Variables" table.');
  ...
  if (value === undefined || value === '' || value === '—') fail(`Variable (${tc}, ${name}) has no value — fill it in the data md.`);
  variables[tc][name] = value;
}
...
const jsonPath = path.join(dataDir, `${ucId}_testdata.json`);
