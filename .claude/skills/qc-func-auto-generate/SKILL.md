---
name: qc-func-auto-generate
description: "Generates the Playwright + TypeScript UI automation project from the audited UC and the test-case md, crawling the running system for locators. Trigger: /qc-func-auto-generate <UC-ID>, 'tạo automation', 'viết test script' — or the LEGACY command '/qc-auto-generate' (old name; routes here). To RUN tests use qc-auto-run; for API scripts use qc-api-auto-generate."
---

# Skill: qc-func-auto-generate
...
- `func-test-cases-md` — latest `.md` (`*_testcases_<variant>_<YYYYMMDD>_v<N>.md`, highest version; never the xlsx — md is the machine-readable source).
...
bugs/                           owned by qc-bug-report / qc-bug-verify (bug report per UC; user updates the Trạng thái column)
...
- SOLE writer of everything under `docs/qc/automation/` except: `data/*_testdata.md` values (QC edits), `portals/*/notification-channels.ts` (QC may hand-edit), the "Trả lời của QC/dev" column in `crawl-findings/` (user fills), `.env` (user-owned), `reports/` (owned by `qc-auto-run`), `bugs/` (owned by `qc-bug-report` / `qc-bug-verify`; user updates the Trạng thái column), and the API branch's areas owned by `qc-api-auto-generate` — `api-findings/`, `runner/services/`, `runner/schemas/`, `runner/tests/api/`, `runner/tests/mix/`, `runner/helpers/api/`, `data/*_api_testdata.*` — this skill NEVER writes there.
