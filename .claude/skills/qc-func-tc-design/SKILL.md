...
- If the user selects **update-test-cases**, check if the `docs/QC-REPORT/testcases` directory exists or provided by user. If it exists, load `workflows/update-test-cases.md`. If it does not exist, ask the user to provide the `docs/QC-REPORT/testcases` directory.

## Input Contract
For **generate-test-cases** workflow:
- The `uc-review` readiness report - read the latest version
- (Optional) Scenarios generated by `scenario-design` - read the latest version
- All the common files in docs\BA\SRS-report\CMR folder.

For **update-test-cases** workflow:
- The `testcases` directory - provided by user
- The `uc-review` readiness report - read the latest version
- (Optional) Scenarios generated by `scenario-design` - read the latest version
- All the common files in docs\BA\SRS-report\CMR folder.

## Output Contract
All output files should be saved in the `docs/QC-REPORT/testcases/[MOD]/` directory.
For **generate-test-cases** workflow:
| Artifact        | Output Path                                                                 |
| --------------- | --------------------------------------------------------------------------- |
| Test Cases Draft| `[UC-ID]_[feature-name]_testcases_draft.md`                                 |
| Test Cases      | `[UC-ID]_[feature-name]_testcases_[YYYYMMDD]_v[N].xlsx`                     |
| Summary         | `[UC-ID]_[feature-name]_testcases_summary_[YYYYMMDD]_v[N].md`               |

For **update-test-cases** workflow:
| Artifact           | Output Path                                                                 |
| ---------------    | --------------------------------------------------------------------------- |
| Test Cases updated | `[UC-ID]_[feature-name]_testcases_[YYYYMMDD]_v[N].xlsx`                     |
| Summary            | `[UC-ID]_[feature-name]_testcases_summary_[YYYYMMDD]_v[N].md`               |
