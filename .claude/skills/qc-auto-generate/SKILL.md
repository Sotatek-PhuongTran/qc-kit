...
helpers/                        base-page.ts, test-data.ts, email.ts (dynamic email data)
...
data/<UC-ID>_testdata_<date>_v<N>.md      human-editable source (+ compiled .json — build output)
...
- SOLE writer of everything under `docs/qc/automation/` except: `data/*_testdata_*.md` values (QC edits), `.env` (user-owned), `reports/` (owned by `qc-auto-run`).
...
- Generated code files are the versioning exception to `naming-convention.md` (in-place, git-tracked, stamped); authored md files (test data, triage) stay versioned `v<N>`.
