...
| `qc-site-map`              | `project-context-master` (HARD REQ), `High-level-files`, wireframe folder, role matrix, SRS/spec | `qc-site-map`, handoff file `.claude/skills/qc-dashboard-sync/inbox/site-map-handoff.md`. Init → auto-invoke `qc-dashboard-sync`. Update có change → đề xuất user gọi `qc-dashboard-sync`. |
| `qc-dashboard-sync`        | Top-down: `project-context-master`, `.claude/skills/qc-dashboard-sync/inbox/site-map-handoff.md`, `qc-dashboard` (or creates from template), `requirement-files`, `uc-review-report`, `func-test-scenarios`, `func-test-cases-draft`, `func-test-cases`, `requirement-common-files` (exclusion). Bottom-up: same except no handoff. | `qc-dashboard` (owner; writes cols 1-6). NEVER writes cols 7-10. READS `site-map-handoff.md` only — does NOT delete it (lifecycle owned by `qc-site-map`). |
| `qc-uc-read`               | `requirement-common-files`, `requirement-files`, `question-backlog`, `project-context-master`, `qc-site-map` (optional), `qc-dashboard` (precheck) | `uc-review-report`, `question-backlog`, `qc-dashboard` col 7 `UC review stt`. MUST auto-trigger `qc-dashboard-sync` first if UC ID is not in dashboard. |
| `qc-func-scenario-design`  | `uc-review-report`, `project-context-master`, `qc-site-map` (optional), `qc-dashboard` (precheck) | `func-test-scenarios`, `qc-dashboard` col 8 `Scenario design stt`. MUST auto-trigger `qc-dashboard-sync` first if UC ID is not in dashboard. |
| `qc-func-tc-design`        | `uc-review-report`, `func-test-scenarios`, `project-context-master`, `qc-site-map` (optional), `qc-dashboard` (precheck) | `func-test-cases`, `func-test-cases-draft`, `func-test-cases-summary`, `qc-dashboard` col 9 `TC design stt`. MUST auto-trigger `qc-dashboard-sync` first if UC ID is not in dashboard. |
