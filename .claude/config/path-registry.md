...
| `qc-context-master`        | `High-level-files`, `requirement-common-files`, `project-context-master`, `qc-dashboard` | `project-context-master`, `qc-dashboard` (cột 1-5 only) |
...
- **Ngoại lệ versioning:** các file meta-config (`.claude/config/project-config.md`, `.claude/config/path-registry.md`) và các file cố định trong `docs/qc-lead/` (`project-context-master.md`, `qc-dashboard.md`, `agent-work-log.md`) được edit **in-place** vì có path cố định. Không tạo `v<N+1>`.
