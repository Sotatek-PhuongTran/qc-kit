...
Follow `.claude/rules/qc-writting-rules.md` (MANDATORY — văn phong, bảng quy đổi thuật ngữ §3, chống mã trần R2). Re-number Section 1–5 below as §F.1–§F.5 in the output.
...
Each AC row should be self-contained per `.claude/rules/qc-writting-rules.md` R1 (câu tự chứa) and include the source tag.
