...
1. `project-context-master.md` §1 → Product Platform Type. Load the matching interaction vocabulary (web/desktop: Click/Hover/Right-click; mobile native: Tap/Long-press/Swipe/Pinch/Hardware-back).
...
3. Read the highest-version `uc-review-report` for `<UC-ID>`: UC IDs + names in scope, functions per UC, main/alternative/exception flows, business rules + validations (verbatim wording), acceptance criteria (§8), actors/roles/permissions, pre/postconditions, API endpoints + UI states.
...
5. **Build the coverage matrix** in working memory: rows = UCs in scope; columns = the 9 coverage areas (scenario-rules §Coverage rules). Mark each cell `to-cover` / `blocked` (audited KA ⚠️ Missing / ⚡ Partial — surface in Out-of-Scope Flags, do NOT fabricate) / `out-of-scope` (performance/load/security).
6. If the audited Verdict is `NOT READY` → STOP and ask whether to proceed (scenarios will inherit known gaps). Never silently continue.
