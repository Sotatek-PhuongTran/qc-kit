...
**Tác giả:** QC Agent
**Phiên bản:** v2.0
**Tài liệu tham chiếu:** UC55_ChuyenTrangDauTu.md, UC55_chuyen-trang-dau-tu_audited_20260508_v2.md, CMR_Mobile.md, 4 wireframe images
**Changelog v2.0:** Bổ sung scenarios từ wireframe cross-check (Q17: tab vs scroll), BVA mở rộng cho KPI format, edge cases mới (tagline null, State Persistence, Android back button, KPI=0 EP)
...
### Scenario ID: TS_UC55_103
**Scenario Title:** [Wireframe Conflict] Tab navigation vs Scroll dọc liên tục — Xác minh cấu trúc điều hướng màn hình chi tiết
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-UI-DETAIL, Q17
**Test Type:** UI
**Description:** Wireframe "Container các lựa chọn chuyển tab trong 1 chuyên trang" gợi ý màn hình chi tiết có cơ chế tab navigation để chuyển giữa các section, nhưng UC55 Section 2.2 mô tả layout dạng scroll dọc liên tục. Khi mở màn hình chi tiết, xác minh hệ thống sử dụng một trong hai cơ chế: (a) scroll dọc liên tục (theo UC) hoặc (b) tab navigation (theo wireframe). Ghi nhận cơ chế thực tế để đối chiếu với BA.
**Test Focus:** UI State — Wireframe conflict

---

### Scenario ID: TS_UC55_104
**Scenario Title:** [Wireframe Conflict] Tab navigation — Kiểm tra chuyển tab giữa các section (nếu có tab)
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-UI-DETAIL, Q17
**Test Type:** UI
**Description:** Nếu màn hình chi tiết sử dụng tab navigation (theo wireframe), xác minh: (1) Tất cả tab hiển thị đúng tên section (Tổng quan, Lĩnh vực khuyến khích, Hạ tầng KCN, Vị trí địa lý, Liên hệ), (2) Tap vào từng tab chuyển đúng nội dung, (3) Tab đang active có highlight/indicator, (4) Nội dung trong mỗi tab đầy đủ theo UC55.
**Test Focus:** UI State — Wireframe conflict (conditional)

---

### Scenario ID: TS_UC55_105
**Scenario Title:** [Wireframe Conflict] Tab navigation — Sticky tab bar khi scroll nội dung trong tab
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-UI-DETAIL, Q17
**Test Type:** UI
**Description:** Nếu có tab navigation, khi nội dung bên trong 1 tab dài hơn viewport (ví dụ: tab Hạ tầng KCN có nhiều KCN), xác minh tab bar luôn sticky ở vị trí cố định (không bị cuộn theo nội dung), cho phép người dùng chuyển tab bất cứ lúc nào.
**Test Focus:** UI State — Wireframe conflict (conditional)
...
**Test Type:** Functional
**Description:** Khi API trả về GRDP = 0 (không phải null), xác minh thẻ KPI GRDP hiển thị "0%" chứ không phải "--" (dấu hiệu null). Phân biệt rõ ràng giữa giá trị 0 (hợp lệ, hiển thị "0%") và giá trị null (không có dữ liệu, hiển thị "--").
...
**Test Type:** Functional
**Description:** Khi API trả về Dân số = 0, xác minh thẻ hiển thị "0" chứ không phải "--". Phân biệt 0 (dữ liệu hợp lệ) vs null (không có dữ liệu).
**Test Focus:** Boundary — EP (0 vs null)
...
**Test Type:** Functional
**Description:** Khi API trả về Vốn đầu tư = 0, xác minh thẻ hiển thị "$0" chứ không phải "--".
**Test Focus:** Boundary — EP (0 vs null)
...
**Test Type:** Functional
**Description:** Khi API trả về Diện tích = 0, xác minh thẻ hiển thị "0 km²" chứ không phải "--".
**Test Focus:** Boundary — EP (0 vs null)
...
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-UI-SEARCH, Q4, CMR-01
**Test Type:** Functional
**Description:** Theo CMR-01 State Persistence: Sau khi người dùng nhập keyword tìm kiếm tỉnh (ví dụ: "Bắc"), tap vào item tỉnh để xem chi tiết, rồi nhấn Back quay lại danh sách, xác minh: (1) Ô tìm kiếm vẫn hiển thị keyword "Bắc", (2) Danh sách vẫn lọc theo keyword "Bắc" (không reset về 63 tỉnh). Ghi nhận hành vi thực tế để đối chiếu Q4.
...
**Req-ID:** UC55-UI-SEARCH, Q4, CMR-01
**Test Type:** Functional
**Description:** Khi người dùng nhập keyword tìm kiếm, sau đó điều hướng ra khỏi UC55 (quay về Home), rồi vào lại "Khu vực đầu tư", xác minh ô tìm kiếm reset về trạng thái rỗng và hiển thị đủ 63 tỉnh.
...
### Scenario ID: TS_UC55_124
**Scenario Title:** Partial API failure — Chỉ API KCN fail, các section khác thành công
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-ERR-001, Q8
**Test Type:** Integration
**Description:** Khi màn hình chi tiết gọi nhiều API cho từng section (KPI, Tổng quan, Lĩnh vực, KCN, Vị trí, Liên hệ) và chỉ API Hạ tầng KCN trả lỗi 500, xác minh: (a) Các section khác vẫn hiển thị bình thường, (b) Section KCN hiển thị thông báo lỗi cục bộ hoặc (c) Toàn bộ màn hình hiển thị lỗi. Ghi nhận hành vi thực tế để đối chiếu Q8.
**Test Focus:** Error/Exception — Partial failure

---

### Scenario ID: TS_UC55_125
**Scenario Title:** Partial API failure — Chỉ API KPI fail, banner + tổng quan thành công
**UC Reference:** UC55 — Xem chuyên trang đầu tư theo khu vực đầu tư
**Req-ID:** UC55-ERR-001, Q8
**Test Type:** Integration
**Description:** Khi API KPI trả lỗi nhưng API banner/tổng quan thành công, xác minh: (a) Banner vẫn hiển thị hình ảnh + tên tỉnh, (b) 4 thẻ KPI hiển thị "--" cho tất cả hoặc hiển thị thông báo lỗi cục bộ, (c) Các section phía dưới vẫn hiển thị bình thường.
**Test Focus:** Error/Exception — Partial failure
...
| TS_UC55_103–105 | Tab vs Scroll layout | Q17 | Wireframe conflict — chờ BA xác nhận design cuối cùng |
| TS_UC55_116 | Tagline null trên banner | Q9 | Chờ BA xác định hành vi overlay khi tagline rỗng |
| TS_UC55_117 | State Persistence search | Q4 | Chờ BA xác nhận có áp dụng CMR-01 State Persistence |
| TS_UC55_124–125 | Partial API failure | Q8 | Chờ BA xác nhận xử lý khi chỉ 1 API fail |
