## Convert Test Cases MD → XLSX (Phase 2 — Auto-triggered Workflow)

> **Trigger:** This workflow is auto-triggered by `SKILL.md` → Step B, after Phase 1 (`.md` file written) finishes. Do NOT invoke this workflow directly outside that orchestration.
>
> **Phase mapping:** this entire file IS Phase 2 — MD → XLSX Conversion (Phase 1 = design, per `SKILL.md` → Step B).
>
> **Checkpoint references:** all phase-boundary write/update steps follow `.claude/config/checkpoint-protocol.md` + `workflows/checkpoint-protocol.md`. Do NOT duplicate those rules here.
...
## Phase 2 — MD → XLSX Conversion

### Phase 2 Multi-variant Loop Overview

Phase 2 is **per-variant**. It executes in two passes:

**Pass A — Verification Gate (Step 0):** for EACH variant listed in the `## Phase 1 Summary` block (top-level `**Variants in scope:**` line), run Step 0.1 → 0.2 → 0.3. If ALL variants pass (with auto-recovery if needed) → advance `last_phase_done` to `1` ONCE for the whole UC and proceed to Pass B. If ANY variant fails irrecoverably → STOP and do NOT advance `last_phase_done`.
...
This step is the **single gate** that decides whether Phase 1 truly produced a complete final md for every variant. It compares each variant's deliverable md against that variant's `### Variant: <name>` sub-block in the `## Phase 1 Summary` of `progress.md`. Mismatches trigger auto-recovery (overwrite the variant's same-version md from its scratch); irrecoverable failures STOP the skill.

This step is per `workflows/checkpoint-protocol.md` → "Verified-transition rule" (start-of-phase / transition write): it is what advances `last_phase_done` from empty to `1` for the UC. No other code path in this skill is allowed to advance `last_phase_done` to `1`.

#### Step 0.1 — Read the Phase 1 Summary

1. Open `.claude/skills/qc-func-tc-design/process-logging/<UC-ID>/progress.md`.
2. Locate the `## Phase 1 Summary` block. Parse:
...
3. If the entire `## Phase 1 Summary` block is **missing**:
   - Probe `process-logging/<UC-ID>/` for any `02_designed_tcs_*.md` files.
   - **If at least one scratch exists** → derive a `## Phase 1 Summary` block from the discovered scratches (one `### Variant: <V>` sub-block per scratch found, with `Variants in scope:` listing them all). Write it back to progress.md (atomic). Then jump to Step 0.2 with this derived summary.
   - **If NO scratches exist** → STOP and report on chat: "Phase 1 scratches never persisted (Step 3.5 didn't run before interrupt). Cannot auto-recover. Restart the skill to re-design from Phase 1." Do NOT advance `last_phase_done`. Do NOT continue Phase 2.
...
> **TC ID pattern caveat:** The pattern `^\| TC_\d+ \|` matches the conventional `TC_001`, `TC_002`, etc. produced by `references/Testcase-refer-*.md`. If a project's `rules/testcase-instruction-rules.md` declares a different TC ID convention (e.g., `TC-01`, `TC_AUTH_001`), adjust this pattern accordingly when running Phase 2 — failing to adjust would make every check fail and trigger spurious auto-recovery. Verify the pattern matches the project's actual convention before relying on this gate.
...
**If ALL variants pass (no auto-recovery needed, OR auto-recovery succeeded for the ones that failed) — advance the UC to Phase 2:** (Per the checkpoint delta transition rule; ordering: progress.md → worklog.)

1. **Update `progress.md`** → `last_phase_done: 1`, `next_phase: 2`, `updated_at: <now>`. (Preserve all existing fields including the `## Phase 1 Summary` block.)
2. **Worklog**: rewrite last entry → `status = "Running (Phase 2)"`. (No new `input` files appended — the Phase 1 final mds were already recorded in `output` at end of Phase 1.)
3. Proceed to Pass B (Step 1 below).

**If a variant Vᵢ FAILS the checks → Auto-recovery for Vᵢ (no user prompt):**
1. Report on chat (informational, not blocking):
   ```
   ℹ️ Phase 2 gate detected mismatch in variant <Vᵢ> between its final md and the Phase 1 Summary — auto-recovering from scratch.
...
2. Check `process-logging/<UC-ID>/02_designed_tcs_<Vᵢ>.md` exists.
   - **If Vᵢ's scratch missing** → STOP and report: "Variant <Vᵢ>: no scratch available. Phase 1 scratch for <Vᵢ> never persisted. Restart the skill to re-design from Phase 1." Do NOT advance `last_phase_done`. Do NOT continue with any remaining variant.
...
5. If Vᵢ's verification **still fails after recovery** → STOP and report: "Variant <Vᵢ>: scratch and final md still mismatch the Phase 1 Summary — possible corruption. Manual inspection required. Expected: <...>; scratch-sourced md: <...>; summary: <...>." Do NOT advance `last_phase_done`. Do NOT run the converter for any variant.
...
- Identify Vᵢ's finalized `.md` file(s) using the `Final MD file(s)` paths from the Phase 1 Summary sub-block:
...
If any prerequisite fails, STOP and report on chat — do NOT silently fix the md. Do NOT run cleanup; Vᵢ's Phase 1 `.md` remains preserved for the user to re-trigger conversion after fixing the root cause. Already-converted variants (if any) keep their xlsx — they are NOT rolled back.
...
Per `workflows/checkpoint-protocol.md` → "Verified-transition rule" (end-of-phase). Phase 2 is the last phase, so `last_phase_done: 2` IS written here (there is no next phase to transition into). This block runs ONCE for the whole UC, AFTER all variants have passed Step 4.

1. **The deliverable `.xlsx` files (one per variant, written in Step 3 and verified in Step 4) ARE the Phase 2 deliverables.** Do NOT write a separate file in `process-logging/`.
2. **Update `progress.md`** → `last_phase_done: 2`, `next_phase: -` (done), `updated_at: <now>`. (Preserve `## Phase 1 Summary` and all other fields.)
3. **Worklog**: rewrite last entry → `status = "Phase 2 done"`. Append ALL variants' `.xlsx` paths to `output` (excluding `process-logging/`).

### Step 5: Hand Back

Return control to `SKILL.md` → Step C (chat-side reporting). The Step C chat report should list ALL variants' `.md` + `.xlsx` paths (one pair per variant), plus any auto-recovery events that occurred during Step 0. After Step C completes, `SKILL.md` → Step D performs final cleanup (mark worklog `Done`, delete `process-logging/<UC-ID>/` folder — the user runs `/qc-dashboard-sync <UC-ID>` to refresh the dashboard). This workflow does NOT write a summary file.
