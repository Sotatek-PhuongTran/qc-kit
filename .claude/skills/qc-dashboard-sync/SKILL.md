...
description: Sole writer of qc-dashboard.md — syncs the feature list and per-UC status columns from disk scans and upstream handoffs. Step 3 of the chain qc-context-master -> qc-site-map -> qc-dashboard-sync. Trigger on /qc-dashboard-sync (full top-down sync), /qc-dashboard-sync <UC-ID> (bottom-up for one UC), "sync dashboard", "cập nhật dashboard", or when auto-invoked by qc-site-map.
---
...
Maintain `qc-dashboard.md` — the QC Lead's per-feature status board. This skill is the **SOLE writer of every column**. It combines three data sources: the feature list handed off by `qc-site-map`, a disk scan of per-UC artifact folders, and per-UC skill checkpoints (for in-progress status).
...
- **Outputs:**
  ...
  - Console report (Vietnamese): new rows, file/status updates, orphans appended, sort decision, summary.
...
`Automation stt` and `Execute stt` are injected only when the automation project folder exists (contracts §2). Manual-only projects keep the 10 baseline columns. Owner of automation artifacts: `qc-auto-generate` (specs) and `qc-auto-run` (run summaries) — this skill only reads them.
...
- Never invents `Site` / `Module` / `Feature name` — top-down takes them from the handoff; bottom-up leaves them blank on new rows and preserves them on existing rows.
- Never auto-sets `In scope? = Removed` (legal user-edit value only). Rows whose folder disappeared keep their scope; `Files stt` reflects what remains.
- Exclusion folders (Common, Shared, `_template`, …) are never added as rows. Non-ID-pattern folders (e.g. `TrangChuDashboard`) ARE added — they are exactly the orphans Mode 3 reconciles.
- Dashboard schema mismatch → STOP and report; no auto-fix beyond the explicit migrations in top-down Phase 0.
...
- Per-UC skills (`qc-uc-read`, `qc-func-scenario-design`, `qc-func-tc-design`, `qc-auto-generate`) never touch the dashboard; they expose status via their checkpoint `progress.md` (in-progress) and versioned output files (done) — both read-only to this skill.
