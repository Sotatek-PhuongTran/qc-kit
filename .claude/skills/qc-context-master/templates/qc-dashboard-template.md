...
- **{{ID_LABEL}}:** Định danh duy nhất. Tên cột này adapt theo cách dự án gọi (`Use Case ID`, `Feature ID`, `Story ID`, ...). **Giá trị phải khớp folder name** trong các path artifact (vd: nếu giá trị là `UC-VOB-001` thì có folder `docs/.../UC-VOB-001/`) để `qc-dashboard-sync` quét status được.
