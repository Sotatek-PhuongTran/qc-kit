# Checkpoint Delta — qc-func-auto-generate

> Title: Checkpoint Delta qc-func-auto-generate | Created: 2026-07-02 | Updated: 2026-07-04 (crawl-findings + channel map) | Author: Claude (QC Kit v3 rebuild) | Version: v2
...
**Scope:** per-UC — checkpoints at `.claude/skills/qc-func-auto-generate/process-logging/<UC-ID>/`. `progress.md` additionally records `workflow: <generate | update>`, the chosen DOM channel, and the portal.
...
| `01_crawl_<page>.md` | Phase 2 | Per-page crawl result: interaction element → primary/alt locator + stability tier + verified mark; channel-map idioms learned in this session. Unresolved elements are NOT parked here — they go straight to the page's `crawl-findings` living file. Written after each page's crawl so an interrupted multi-page crawl resumes per page. |
