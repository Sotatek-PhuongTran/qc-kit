...
**Tác giả:** QC Agent
**Phiên bản:** v3.0
**Tài liệu tham chiếu:** UC55_ChuyenTrangDauTu.md, UC55_chuyen-trang-dau-tu_audited_20260508_v3.md, CMR_Mobile.md, 4 wireframe images
**Changelog v3.0:** Cập nhật sau khi BA trả lời 10/17 câu hỏi. Xóa 6 scenarios (CTA bỏ Q1, wireframe tab resolved Q17, partial API failure Q8). Unblock 4 scenarios (debounce Q3, KPI=0 Q2, State Persistence Q4, timeout Q5). Thêm 7 scenarios mới (map fallback browser, KPI=0 EP cho 3 thẻ, all-null KPI, portrait lock, KPI format UC55-specific). Tổng active: 129 scenarios (135 total, 6 removed).
...
**Test Type:** Functional
**Description:** Xác minh khi chọn một tỉnh (ví dụ: "An Giang"), hệ thống gọi API chi tiết và hiển thị đầy đủ 7 sections theo thứ tự: Banner ảnh → KPI → Tổng quan đầu tư → Lĩnh vực khuyến khích → Hạ tầng KCN → Vị trí địa lý → Liên hệ đầu tư (+ nút "Cổng thông tin đầu tư"). Dữ liệu khớp 100% với API. **[v3]** CTA "Đăng ký tư vấn ngay" đã bỏ.
**Test Focus:** Happy path
...
**Description:** Xác minh luồng đầy đủ: Đăng nhập → Sidebar → "Khu vực đầu tư" → hiển thị 63 tỉnh A–Z → tap "Bắc Ninh" → skeleton loading → hiển thị chi tiết 7 sections → cuộn xem toàn bộ → tap bản đồ (mở Maps) → quay lại → tap "Cổng thông tin đầu tư" (mở browser) → tap Quay lại → trở về danh sách tỉnh.
**Test Focus:** Happy path
...
**Req-ID:** AC4
**Test Type:** Acceptance
**Description:** Xác minh khi API không trả dữ liệu cho từng section: KPI → "--" (4 thẻ vẫn hiển thị), Tổng quan → "Không có dữ liệu", Lĩnh vực → "Không có dữ liệu", KCN → "Không có dữ liệu", Khoảng cách → ẩn (bản đồ vẫn hiện), Liên hệ → "--", URL Cổng thông tin → ẩn nút.
**Test Focus:** Error/Exception
...
**Test Type:** Functional
**Description:** Xác minh khi GRDP = 0, thẻ hiển thị "0.0%" (1 chữ số thập phân). Giá trị 0 khác null: null → "--", 0 → hiển thị giá trị bình thường. 4 thẻ KPI luôn hiển thị đủ (Q2 — Answered).
**Test Focus:** Boundary
...
**Test Type:** Functional
**Description:** Xác minh khi Dân số = 0, thẻ hiển thị "0" (số nguyên, không rút gọn). Giá trị 0 khác null: null → "--", 0 → hiển thị giá trị bình thường. 4 thẻ KPI luôn hiển thị đủ (Q2 — Answered).
**Test Focus:** Boundary — EP (0 vs null)
...
**Req-ID:** UC55-FR-007, BR-06, BR-11
**Test Type:** Functional
**Description:** Xác minh khi Vốn đầu tư = 0, thẻ hiển thị "$0" (không rút gọn). Giá trị 0 khác null: null → "--", 0 → hiển thị giá trị bình thường. 4 thẻ KPI luôn hiển thị đủ (Q2 — Answered).
**Test Focus:** Boundary — EP (0 vs null)
...
**Req-ID:** UC55-FR-007, BR-06, BR-11
**Test Type:** Functional
**Description:** Xác minh khi Diện tích = 0, thẻ hiển thị "0 km²" (số nguyên). Giá trị 0 khác null: null → "--", 0 → hiển thị giá trị bình thường. 4 thẻ KPI luôn hiển thị đủ (Q2 — Answered).
**Test Focus:** Boundary — EP (0 vs null)
...
**Test Type:** Functional
**Description:** Xác minh khi API trả về null cho cả 4 KPI (GRDP, Dân số, Vốn đầu tư, Diện tích), hệ thống hiển thị đủ 4 thẻ với giá trị "--" cho tất cả. Layout không bị vỡ, không ẩn section KPI (Q2 — Answered: 4 thẻ luôn hiển thị đủ).
