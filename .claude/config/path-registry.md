...
| QC      | `test-data`                | `docs/qc/automation/data/` | `<UC-ID>_testdata_<YYYYMMDD>_v<N>.md` | Nguồn data test (human-edit): accounts theo vai trò, biến theo TC, precondition-data, config. KHÔNG chứa mật khẩu |
| QC      | `test-data-json`           | `docs/qc/automation/data/` | `<UC-ID>_testdata.json` | Fixture JSON đã validate — **build output** từ `test-data` md (qua `scripts/build-test-data.mjs`); KHÔNG sửa tay |
