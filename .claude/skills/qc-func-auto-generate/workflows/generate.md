# qc-func-auto-generate — Generate workflow
...
2. **Hard prerequisites** (resolve via `path-registry.md`); missing → STOP with a Vietnamese message naming exactly what is missing:
   ...
   - `func-test-cases-md` latest `.md` (`*_testcases_<variant>_<YYYYMMDD>_v<N>.md`, highest version — NOT the xlsx; md is the machine-readable source).
...
In `package.json`, substitute the `"name"` placeholder `<project>-automation` with the project slug (from `project-context-master`, lowercase-kebab).
...
**`.env.example` scaffold (placeholders only — NEVER values, NEVER URLs, NEVER passwords):** ensure `.env.example` at the runner root lists the secret keys the scaffolded helpers read — when `helpers/email.ts` is scaffolded: `MAILTRAP_API_TOKEN`, `MAILTRAP_ACCOUNT_ID`, `MAILTRAP_INBOX_ID` (exact keys `helpers/email.ts` reads; per-env values may use the env-name prefix, e.g. `QA_MAILTRAP_API_TOKEN` — see `../references/data-and-secrets.md`); plus the DB connection key when an L4 seed script is scaffolded (Phase 2 step 4). Append missing keys only; never overwrite existing lines, never write a secret.
